...
from models.base_networks import DFFBlock, UBPBlock, DBPBlock
...
class LREncoder(nn.Module):
    ...
    def forward(self, x):
        fm_32 = self.down1(x)
        fm_64 = self.down2(fm_32)
        fm_output = self.conv_output(fm_64)
        return fm_output
...
class ResEncoder(nn.Module):
    ...
    def forward(self, fm_res_in):
        fm_8 = self.conv_output(fm_res_in)
        fm_resize = fm_8.view(fm_8.shape[0], -1)
        h = self.fc(fm_resize)
...
class ResDecoder(nn.Module):
    def __init__(self, dim_z=128):
        ...
        self.fusion = nn.Conv2d(16, 8, 1, 1, 0)

        self.conv_output = nn.Sequential(
            nn.Conv2d(8, 16, 1, 1, 0),
            nn.ConvTranspose2d(16, 32, 6, 2, 2),
            ResidualBlock(32),
            nn.ConvTranspose2d(32, 64, 6, 2, 2),
            ResidualBlock(64)
        )

    def forward(self, z, fm_LR_out):
        fm_input = self.fc(z)
        fm_resize = fm_input.view(z.shape[0], 8, 16, 16)

        fm_concat = torch.concat((fm_resize, fm_LR_out), 1)
        fm_fusion = self.fusion(fm_concat)
        fm_res = self.conv_output(fm_fusion)
        return fm_res
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.fusion = nn.Conv2d(16, 16, 1, 1, 0)
    ...
    def forward(self, fm_res, fm_LR_in):
        fm_32 = self.upConv1(fm_res)
        fm_16 = self.upConv2(fm_32) + self.fusion(fm_LR_in)

        out = self.conv_output(fm_16)
        return out
...
class SR_CyGen(nn.Module):
    def __init__(self, opt):
        ...
        self.conv_input_HR = ConvInput()
        self.conv_input_LR = ConvInput()
        self.fusion = nn.Conv2d(16, 16, 1, 1, 0)
        self.encoder_HR = HREncoder()
        self.encoder_LR = LREncoder()
    ...
    def setLR(self, LR):
        self.LR = LR
        self.fm_LR_in = self.conv_input_LR(self.LR)
        self.fm_LR_out = self.encoder_LR(self.fm_LR_in)
    ...
    def eval_logp(self, fm_res_in, z):
        self.fm_res_out = self.resDecoder(z, self.fm_LR_out)
    ...
    def getlosses(self, LR, HR):
        self.setLR(LR)
        fm_HR_in = self.conv_input_HR(HR) - self.fusion(self.fm_LR_in)
        fm_res_in = self.encoder_HR(fm_HR_in)
        ...
        SR_predict = self.decoder(self.fm_res_out, self.fm_LR_in)
    ...
    def generate(self, gentype, LR, n_iter1=6, n_iter2=10):
        result = []
        self.setLR(LR)
        fm_HR_in = self.conv_input_HR(LR) - self.fusion(self.fm_LR_in)
        fm_res_in = self.encoder_HR(fm_HR_in)
        ...
        for i in range(n_iter1):
            ...
            SR_pred = self.decoder(fm_res_out, self.fm_LR_in)
            fm_HR_in = self.conv_input_HR(SR_pred) - self.fusion(self.fm_LR_in)
            fm_res_in = self.encoder_HR(fm_HR_in)
            xp = fm_res_in
