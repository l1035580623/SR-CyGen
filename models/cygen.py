    ...
    def generate(self, gentype, draw_p, n_iter = None, z0 = None, x0 = None,
        ...
        if gentype == "gibbs":
            with tc.no_grad() if no_grad else suppress():
                z = self.draw_q(x0, 1) if z0 is None else z0
...
class SampleGradContainer_FlowqNoInv:
    ...
    def eval_if_hasnot(self, x, eps = None, track_x = True, track_eps = True, track_z = True):
        ...
        if eps is None:
            with tc.no_grad():
                eps = self.draw_q0(x.shape[0]).squeeze(0) # without reparameterization
        with tc.enable_grad():
...
class CyGen_FlowqNoInv(CyGen):
    def __init__(self, cmtype, pxtype, eval_logp, draw_q0, eval_z1eps_logqt, eval_z1eps=None):
        ...
        self.draw_q0 = draw_q0      # 生成随机向量
        self.eval_z1eps_logqt = eval_z1eps_logqt    # encoder + logqt
        self.eval_z1eps = eval_z1eps    # encoder
        self._sgc_rho = self._sgc_q = SampleGradContainer_FlowqNoInv(eval_logp, draw_q0, eval_z1eps_logqt)
        if eval_z1eps is None:
            self.draw_q = lambda x, n_mc: eval_z1eps_logqt(x, draw_q0(x.shape[0]))[0]
        else:
            self.draw_q = lambda x, n_mc: eval_z1eps(x, draw_q0(x.shape[0]))
    ...
    def generate(self, gentype, draw_p, n_iter = None, z0 = None, x0 = None,
        ...
        if gentype in {"gibbs", "xhat", "langv-x"}:
            ...
            with tc.no_grad() if no_grad else suppress():
                ...
                batchSize = x0.shape[0] if z0 is None else z0.shape[0]
                eps = self.draw_q0(batchSize).squeeze(0) if eps0 is None else eps0.clone() # If `no_grad`, then no need of `detach`
                sgc = self._sgc_q
                for itr in range(n_iter):
